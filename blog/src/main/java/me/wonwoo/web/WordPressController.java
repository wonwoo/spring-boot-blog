...
public class WordPressController {
  ...
  @GetMapping
  public String findAll(@ModelAttribute SearchForm searchForm, Model model,
                        @PageableDefault(size = 3, sort = "post_date", direction = Sort.Direction.DESC)
                            Pageable pageable) {
    Page<WpPosts> wpPostses = postElasticSearchService.wpPosts(pageable);
    List<WpPosts> wpPosts = wpPostses.getContent()
        .stream()
        .peek(content -> content.setPostContent(pegDownProcessor
            .markdownToHtml(unescapeHtml(content.getPostContentFiltered()))))
        .collect(toList());
  ...
  public String search(Model model, @PageableDefault(size = 3) Pageable pageable,
    ...
    model.addAttribute("wordPresses",
        new PageImpl<>(wpPosts, pageable, wpPostses.getTotalElements()));
